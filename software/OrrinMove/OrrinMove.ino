...
//  Front Right LEG
const int FRThigh = 3;
const double FRThighOffset = 1.2;
const int FRCalf = 4;
const double FRCalfOffset = 1.0;
const int FRHip = 5;
...
// Back Right LEG
const int BRThigh = 9;
const double BRThighOffset = 1.2;
const int BRCalf = 10;
...
void loop() {
  ...
  Serial.print(tempY);
  Serial.print("thigh: ");
  Serial.println(convertThighAngleToServo(angles[3][0]));
  Serial.print("calf: ");
  Serial.println(convertCalfAngleToServo(angles[3][1]));
...
void moveOrrin(double x0, double y0, double r0,
  ...
  // CALF
  HCPCA9685.Servo(FLCalf, (convertCalfAngleToServo(angles[0][1]) + 70) * FRCalfOffset);
  // HIP
  HCPCA9685.Servo(FLHip, angles[0][2]);

  //  Front Right LEG
  // THIGH
  HCPCA9685.Servo(FRThigh, convertThighAngleToServo(angles[1][0]) * FRThighOffset);
  // CALF
  ...
  //  Back Left LEG
  // THIGH
  HCPCA9685.Servo(BLThigh, altConvertThighAngleToServo(angles[2][0]) * BLThighOffset);
  // CALF
  HCPCA9685.Servo(BLCalf, (convertCalfAngleToServo(angles[2][1]) + 70) * BLCalfOffset);
  // HIP
  HCPCA9685.Servo(BLHip, angles[2][2]);

  //  Back Right LEG
  // THIGH
  HCPCA9685.Servo(BRThigh, (convertThighAngleToServo(angles[3][0]) + 60) * BRThighOffset);
  // CALF
  HCPCA9685.Servo(BRCalf, altConvertCalfAngleToServo(angles[3][1]) * BRCalfOffset);
  // HIP
